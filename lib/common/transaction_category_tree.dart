...
class TransactionCategoryTree extends StatefulWidget {
  ...
  const TransactionCategoryTree({super.key, this.itemTap, required this.categories});
  ...
  Widget build(BuildContext context) {
    ...
    return ListenableBuilder(
        ...
        builder: (context, child) {
          return TreeView<TransactionCategory>(
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              ...
              // See also: ExpandIcon.
              IconButton(
                onPressed: entry.hasChildren ? onTap : () {},
                icon: Icon(entry.node.icon),
                color: theme.iconTheme.color,
              ),
              Text(entry.node.name),
              const SizedBox(width: 10),
              if (entry.node.child.isNotEmpty) Text('(${entry.node.child.length})'),
            ],
          ),
        ),
      ),
